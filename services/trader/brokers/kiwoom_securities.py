...
OFFICIAL_BASE_URL = "https://openapi.kiwoom.com"
...
class KiwoomSecuritiesClient:
    ...
    @classmethod
    def from_env(cls, *, transport=None) -> "KiwoomSecuritiesClient":
        return cls(
            client_id=os.getenv("KIWOOM_SECURITIES_CLIENT_ID", ""),
            client_secret=os.getenv("KIWOOM_SECURITIES_CLIENT_SECRET", ""),
            account_seq=os.getenv("KIWOOM_SECURITIES_ACCOUNT_SEQ", ""),
            base_url=os.getenv("KIWOOM_SECURITIES_BASE_URL", OFFICIAL_BASE_URL),
            mode=BrokerMode.parse(os.getenv("KIWOOM_SECURITIES_MODE")),
            allow_live_orders=parse_bool(
                os.getenv("KIWOOM_SECURITIES_ALLOW_LIVE_ORDERS")
            ),
            allow_custom_base_url=parse_bool(
                os.getenv("KIWOOM_SECURITIES_ALLOW_CUSTOM_BASE_URL")
            ),
            timeout=float(os.getenv("KIWOOM_SECURITIES_TIMEOUT_SECONDS", "10")),
            transport=transport,
        )
    ...
    def _validate_base_url(self, allow_custom: bool) -> None:
        ...
        if not allow_custom:
            raise BrokerConfigError(
                "Custom Kiwoom Securities base URLs require "
                "KIWOOM_SECURITIES_ALLOW_CUSTOM_BASE_URL=true."
            )
        if parsed.scheme != "https" and not (parsed.scheme == "http" and is_loopback):
            raise BrokerConfigError(
                "Custom Kiwoom Securities base URL must use HTTPS or 127.0.0.1 HTTP."
            )
    ...
    def _require_read(self) -> None:
        if not self.mode.allows_read:
            raise BrokerConfigError(
                "Kiwoom Securities is disabled. Set KIWOOM_SECURITIES_MODE=read_only "
                "or paper to enable read operations."
            )
        if not self.client_id or not self.client_secret:
            raise BrokerConfigError(
                "KIWOOM_SECURITIES_CLIENT_ID and KIWOOM_SECURITIES_CLIENT_SECRET are required."
            )
        ...
        with self._token_lock:
            ...
            if not token or expires_in <= 0:
                raise BrokerApiError(
                    "Kiwoom Securities returned an invalid OAuth token response.",
                    code="invalid-token-response",
                    status_code=getattr(response, "status_code", None),
                )
            self._token = KiwoomToken(token, now + expires_in)
            return token
    ...
    def _headers(self, *, account: bool = False) -> dict[str, str]:
        ...
        if account:
            if not self.account_seq:
                raise BrokerConfigError(
                    "KIWOOM_SECURITIES_ACCOUNT_SEQ is required for account and order APIs."
                )
            headers["X-Kiwoom-Account"] = self.account_seq
        return headers
    ...
    def _parse_response(self, response, *, oauth: bool = False) -> dict[str, Any]:
        ...
        if oauth:
            ...
            message = str(
                ...
                or "Kiwoom Securities OAuth request failed."
            )
            request_id = headers.get("X-Request-Id")
        else:
            error = payload.get("error") if isinstance(payload.get("error"), dict) else {}
            code = str(error.get("code") or "broker-api-error")
            message = str(error.get("message") or "Kiwoom Securities API request failed.")
